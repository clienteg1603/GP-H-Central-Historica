...
print(f'Database protegido: {len(base_db)} métodos AST-idênticos', flush=True)
...
# Nenhum timer/modal deve participar do smoke; queremos somente geometria Tk.
...
try:
    ...
    app.show_home()
    app.update_idletasks()
    print('Home construída', flush=True)
    ...
    print(f'Home reduzida rolável: conteúdo={content_h}px viewport={viewport_h}px yview={first2:.3f}-{last2:.3f}', flush=True)
    ...
    # event_generate executa os bindings de forma síncrona; update_idletasks
    # basta para a geometria do tooltip e não dispara timers do aplicativo.
    ...
    assert int(card.cget('highlightthickness')) == 1
    print('Hover estável: popup preservado e geometria constante', flush=True)
finally:
    try:
        app.destroy()
    except Exception:
...
print('Smoke v0.46.2 OK', flush=True)
